  ...
  properties: {
    ...
    designLayers: {
      ...
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['type', 'placement', 'appliesTo', 'verticalSize', 'description'],
        properties: {
          ...
          verticalSize: { type: 'string', enum: [...referenceSpec.VERTICAL_SIZES] },
...
function analysisInstructions(role) {
  ...
  return [
    ...
    'Estimate each layer height from its visible proportion on the garment; do not normalize every panel to a standard band.',
    'Use verticalSize: trim_1_3 for lace/piping or very slim borders, narrow_3_5, standard_6_8 for a normal panel, broad_8_10 for a moderately broad panel, extra_broad_10_16 for a clearly tall panel, deep_16_plus when it occupies a substantial lower section of the garment, and not_applicable only for non-band elements.',
    'If a panel is unusually deep, preserve that evidence even when it approaches or exceeds one quarter of the visible ghaghro height.',
