...
function buildPrompt(selections, {
  ...
  const refPhrase = identityCount > 0
    ? `Use the first ${identityCount} attached reference photo${identityCount === 1 ? '' : 's'} as the ground truth for Fatema's identity.`
    : '';
  ...
  const baseClothPhrase = hasBaseClothReference
    ? "Use the first non-identity attached image strictly as the base-cloth reference. Reproduce that cloth's actual colors, print, motif scale, spacing, weave, sheen, and texture across both the pardi and ghagra. Ignore people, hands, labels, shop fixtures, folds, shadows, and background objects in the cloth photo."
    : baseDescription
  ...
  const designReferencePhrase = hasDesignReference
    ? "Use the final attached image strictly as a tailoring-design reference. Copy its panel, lace, border, and embroidery arrangement—not its person, face, body, base cloth, colors, setting, or garment type. Adapt that design to the authentic rida structure."
    : '';
