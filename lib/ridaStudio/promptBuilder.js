...
function buildPrompt(selections, {
  ...
  const refPhrase = identityCount > 0
    ? identityCount === 1
      ? "Use attached reference image 1 as the ground truth for Fatema's identity."
      : `Use attached reference image 1 as the primary facial reference for Fatema. Use reference images ${supportingIdentityStartIndex} through ${supportingIdentityStartIndex + identityCount - 2} as supporting identity views of the same woman.`
    : '';
  ...
  const baseClothPhrase = hasBaseClothReference
    ? `MANDATORY FABRIC MATCH: attached reference image ${baseClothReferenceIndex} is the base-cloth source. Reproduce that exact cloth's dominant and secondary colors, print, motif shapes, motif scale, spacing, weave, sheen, and texture across broad, clearly visible areas of both the pardi and ghagra. Do not substitute a similar fabric, simplify its pattern, recolor it, or invent a different print. Both generated candidates must visibly use this same uploaded cloth. Ignore people, hands, labels, shop fixtures, folds, shadows, and background objects in the cloth photo.`
    : baseDescription
      ...
      : surpriseColor || surpriseMotif
        ? [
          ...
          'The two candidates must be visibly different from each other and from every identity-reference outfit while remaining coordinated across each candidate\'s pardi and ghagra.',
        ].join(' ')
  ...
  const designReferencePhrase = hasDesignReference
    ? `Attached reference image ${designReferenceIndex} is strictly the tailoring-design reference. Copy its panel, lace, border, and embroidery arrangement—not its person, face, body, base cloth, colors, setting, or garment type. Adapt that design to the authentic rida structure while preserving the mandatory uploaded base cloth.`
    : '';
  ...
  const selectedDesignPhrase = !hasDesignReference && !designDescription
    ? surprisePanel || surpriseBorder
      ? [
        ...
        'Make the two candidates visibly different in their decorative design details.',
