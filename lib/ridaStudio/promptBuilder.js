...
const PANEL_SIZING_CLAUSE =
  "When the requested or referenced rida includes decorative panels, render standard panels at 6-8 " +
  "inches of vertical height and broad or wide panels at 8-10 inches of vertical height. Judge those " +
  "dimensions relative to Fatema's 5-foot-10-inch (178 cm) height and the real garment scale. Never " +
  "inflate a panel into an oversized quarter-skirt section; keep it proportionate near the lower garment. " +
  "Do not add a panel when the selected or complete-rida specification has none. When a structured " +
  "reference gives a vertical size for each layer, follow that per-layer size exactly.";
...
function buildPrompt(selections, {
  ...
  const designReferencePhrase = designMode === 'guided' && hasDesignReference
    ? [
      ...
      designAnalysis
        ...
        : '',
      designCorrection
        ? `USER-REQUESTED DESIGN CHANGE: "${designCorrection}". Apply only the requested change and preserve every other analyzed layer, its order, and its placement.`
        : '',
    ].filter(Boolean).join(' ')
    : '';
  ...
  const completeRidaPhrase = designMode === 'complete'
    ? hasCompleteRidaReference
      ? [
        `HIGHEST-PRIORITY COMPLETE-RIDA SOURCE: attached reference image ${completeRidaReferenceIndex} controls the entire generated garment and must be followed as the whole-rida visual specification.`,
        ...
        DESIGN_STACK_CLAUSE,
        "Ignore and never copy the source image's person, face, body, body proportions, age, pose, or background; those are not garment-design references.",
        ...
        completeRidaCorrection
          ? `USER-REQUESTED CHANGE TO THE COMPLETE RIDA: "${completeRidaCorrection}". Treat this as a precise delta: apply only the requested change and preserve every other analyzed base-cloth and design detail from the reference.`
          : '',
