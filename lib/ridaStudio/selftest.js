...
async function main() {
  ...
  test('builds a prompt containing the authentic rida definition and selections', () => {
    ...
    assert.ok(prompt.includes('reference image 1 as the primary facial reference'));
    assert.ok(prompt.includes('reference images 2 through 10 as supporting identity views'));
  ...
  test('base cloth and design each support uploaded, described, or selected modes', () => {
    ...
    test('surprise defaults create fresh designs without copying identity-reference clothing', () => {
      ...
      assert.ok(prompt.includes('primary facial reference only'));
      ...
      assert.ok(prompt.includes('two candidates must be visibly different'));
    });
    ...
    assert.ok(prompt.includes('reference image 3 as the primary facial reference'));
    assert.ok(prompt.includes('reference images 4 through 12 as supporting identity views'));
  ...
  await testAsync('POST /generate prioritizes uploads and limits competing identity references', async () => {
    ...
    assert.ok(lastFakeRequest.prompt.includes('reference image 3 as the primary facial reference'));
    assert.ok(lastFakeRequest.prompt.includes('reference images 4 through 6 as supporting identity views'));
  });
