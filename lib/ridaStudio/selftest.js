...
async function main() {
  ...
  test('complete-rida photo is the highest-priority whole-garment reference', () => {
    const prompt = promptBuilder.buildPrompt({
      ...
      completeRidaReferenceIndex: 1,
      primaryIdentityReferenceIndex: 2,
      supportingIdentityStartIndex: 3,
    });
    assert.ok(prompt.includes('HIGHEST-PRIORITY COMPLETE-RIDA SOURCE'));
    assert.ok(prompt.includes('reference image 1 controls the entire generated garment'));
    ...
    assert.ok(prompt.includes('photo remains controlling wherever the two conflict'));
    assert.ok(prompt.includes('reference image 2 as the primary face-and-body reference'));
  ...
  await testAsync('POST /generate prioritizes uploads and limits competing identity references', async () => {
    const res = await fetch(`${base}/generate`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Cookie: sessionCookie },
      body: JSON.stringify({
    ...
    assert.strictEqual(res.status, 200);
    assert.strictEqual(lastFakeRequest.referenceBuffers.length, 6);
    assert.strictEqual(lastFakeRequest.referenceBuffers[0].filename, 'uploaded-base-cloth-photo.png');
    assert.strictEqual(lastFakeRequest.referenceBuffers[1].filename, 'uploaded-design-example-photo.png');
    assert.ok(lastFakeRequest.prompt.includes('reference image 1 controls'));
    assert.ok(lastFakeRequest.prompt.includes('reference image 2 is strictly the tailoring-design reference'));
    assert.ok(lastFakeRequest.prompt.includes('reference image 3 as the primary face-and-body reference'));
    assert.ok(lastFakeRequest.prompt.includes('reference images 4 through 6 as supporting face-and-body views'));
  });
  ...
  await testAsync('POST /generate puts complete-rida photo first and uses four identity references', async () => {
    const res = await fetch(`${base}/generate`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Cookie: sessionCookie },
      body: JSON.stringify({
    ...
    assert.strictEqual(res.status, 200);
    assert.strictEqual(lastFakeRequest.referenceBuffers.length, 5);
    assert.strictEqual(lastFakeRequest.referenceBuffers[0].filename, 'uploaded-complete-rida-photo.png');
    assert.ok(lastFakeRequest.prompt.includes('reference image 1 controls the entire generated garment'));
    assert.ok(lastFakeRequest.prompt.includes('reference image 2 as the primary face-and-body reference'));
    assert.ok(lastFakeRequest.prompt.includes('reference images 3 through 5 as supporting face-and-body views'));
