...
async function main() {
  ...
  test('base cloth and design each support uploaded, described, or selected modes', () => {
    const prompt = promptBuilder.buildPrompt({
      ...
      baseClothReferenceIndex: 2,
      designReferenceIndex: 3,
    ...
    assert.ok(prompt.includes('MANDATORY FABRIC MATCH'));
    assert.ok(prompt.includes('reference image 2 is the base-cloth source'));
    assert.ok(prompt.includes('areas of both the pardi and ghagra'));
    assert.ok(prompt.includes('reference image 3 is strictly the tailoring-design reference'));
    assert.ok(prompt.includes('reference images 4 through 12 as supporting identity views'));
    assert.ok(prompt.includes('Do not substitute a similar fabric'));
    assert.ok(prompt.includes('Both generated candidates must visibly use this same uploaded cloth'));
  ...
  await testAsync('POST /generate prioritizes cloth and design immediately after the primary identity photo', async () => {
    const res = await fetch(`${base}/generate`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Cookie: sessionCookie },
      body: JSON.stringify({
    ...
    assert.ok(lastFakeRequest.prompt.includes('reference image 2 is the base-cloth source'));
    assert.ok(lastFakeRequest.prompt.includes('reference image 3 is strictly the tailoring-design reference'));
    assert.ok(lastFakeRequest.prompt.includes('reference images 4 through 12 as supporting identity views'));
  });
