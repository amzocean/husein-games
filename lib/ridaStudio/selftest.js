...
async function main() {
  ...
  test('lists options for all curated garment and scene categories', () => {
    ...
    assert.ok(opts.panels.some((item) => item.key === 'coordinatedWide' && item.label.includes('Extra-Broad')));
    assert.ok(options.PANELS.contrastPanel.desc.includes('6-8 inches'));
    assert.ok(options.PANELS.coordinatedWide.desc.includes('10-16 inches'));
  });
  ...
  test('normalizes context-specific structured reference analyses', () => {
    ...
    const deepPanelAnalysis = {
      ...DESIGN_ANALYSIS,
      designLayers: [{
        ...DESIGN_ANALYSIS.designLayers[0],
        verticalSize: 'deep_16_plus',
        description: 'very deep floral lower panel matching the source proportion',
      }],
    };
    assert.deepStrictEqual(
      referenceSpec.normalizeReferenceSpec(deepPanelAnalysis, 'design'),
      deepPanelAnalysis,
    );
    assert.ok(referenceSpec.formatReferenceSpec(deepPanelAnalysis)
      .includes('more than 16 inches high, matching the source proportion'));
    assert.ok(referenceAnalyzer.analysisInstructions('complete')
      .includes('do not normalize every panel to a standard band'));
  });
  ...
  test('builds a prompt containing the authentic rida definition and selections', () => {
    ...
    assert.ok(prompt.includes('Standard panels are 6-8 inches high'));
    assert.ok(prompt.includes('extra-broad panels are 10-16 inches'));
    assert.ok(prompt.includes('unusually deep panels may exceed 16 inches'));
    assert.ok(prompt.includes('Do not normalize a referenced or requested broad panel'));
  });
  ...
  test('base cloth and design each support uploaded, described, or selected modes', () => {
    const prompt = promptBuilder.buildPrompt({
      ...
      designCorrection: 'Keep both panels separate and make the floral panel 15 inches high.',
    }, {
    ...
    assert.ok(prompt.includes('highest-priority design instruction'));
    assert.ok(prompt.includes('Any requested panel size overrides the analyzed size'));
    assert.ok(prompt.includes('visibly increase its vertical height'));
  ...
  test('explicit broad-panel corrections override default panel sizing', () => {
    const prompt = promptBuilder.buildPrompt({
      ...VALID_SELECTIONS,
      designMode: 'complete',
      completeRidaDescription: '',
      completeRidaAnalysis: COMPLETE_ANALYSIS,
      completeRidaCorrection: 'Keep everything else, but make the floral panel much broader and 15 inches high.',
    }, {
      identityReferenceCount: 4,
      hasCompleteRidaReference: true,
      completeRidaReferenceIndex: 2,
    ...
    assert.ok(prompt.includes('Any requested panel size overrides the analyzed size'));
    assert.ok(prompt.includes('visibly increase its vertical height'));
    assert.ok(prompt.includes('explicit user request first'));
    assert.ok(prompt.includes('Do not normalize a referenced or requested broad panel'));
  ...
  await testAsync('POST /generate supports complete description without reducing identity references', async () => {
    ...
    assert.ok(lastFakeRequest.prompt.includes('Standard panels are 6-8 inches high'));
  });
