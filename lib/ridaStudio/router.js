...
function createRouter() {
  ...
  router.post('/generate', requireAuth, requireJsonBody, async (req, res) => {
    ...
    try {
      ...
      // Reference order matters for image-edit fidelity. Keep the primary
      // face first, then place material/design references before the
      // supporting identity views so the model does not underweight them.
      const referenceBuffers = [identityReferences[0]];
      let nextReferenceIndex = 2;
      ...
      referenceBuffers.push(...identityReferences.slice(1));
