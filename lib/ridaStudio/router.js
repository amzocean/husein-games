// Express router for Fatema's Rida Studio — the permanent, PIN-protected
// production game. Mounted by the root server.js at /rida-studio/api.
'use strict';

const express = require('express');
const fs = require('fs');

const session = require('./session');
const rateLimit = require('./rateLimit');
const options = require('./options');
const promptBuilder = require('./promptBuilder');
const referenceAnalyzer = require('./referenceAnalyzer');
const referenceSpec = require('./referenceSpec');
const analysisToken = require('./analysisToken');
const identity = require('./identity');
const tilesPhotos = require('../shared/tilesPhotos');
const openaiImagesClient = require('../shared/openaiImagesClient');

const GENERATE_COUNT = 1;
const IMAGE_SIZE = '1024x1536'; // Portrait, suited to a full-body rida look.
const IMAGE_QUALITY = 'medium';
const SELECTION_FIELDS = [
  'designMode',
  'color', 'motif', 'border', 'panel', 'style', 'location',
  'baseDescription', 'designDescription', 'embroideryDescription',
  'baseClothPhoto', 'designPhoto',
  'completeRidaDescription', 'completeRidaPhoto',
  'baseClothAnalysisToken', 'baseClothCorrection',
  'designAnalysisToken', 'designCorrection',
  'completeRidaAnalysisToken', 'completeRidaCorrection',
];
const RATE_LIMIT_KEY = 'rida-studio-pin-user';
const MAX_FABRIC_BYTES = 5 * 1024 * 1024;
const MAX_IDENTITY_REFERENCES_WITH_UPLOADS = 4;
const FABRIC_MIME_TYPES = new Set(['image/jpeg', 'image/png', 'image/webp']);
const MAX_CONCURRENT_ANALYSES_PER_SESSION = 2;
const MAX_CONCURRENT_ANALYSES_GLOBAL = 3;
const analysesInProgress = new Map();
let globalAnalysesInProgress = 0;

function beginReferenceAnalysis(token, role) {
  const activeRoles = analysesInProgress.get(token) || new Set();
  if (activeRoles.has(role)) return false;
  if (activeRoles.size >= MAX_CONCURRENT_ANALYSES_PER_SESSION) return false;
  if (globalAnalysesInProgress >= MAX_CONCURRENT_ANALYSES_GLOBAL) return false;
  activeRoles.add(role);
  analysesInProgress.set(token, activeRoles);
  globalAnalysesInProgress++;
  return true;
}

function endReferenceAnalysis(token, role) {
  const activeRoles = analysesInProgress.get(token);
  if (!activeRoles) return;
  if (activeRoles.delete(role)) globalAnalysesInProgress--;
  if (!activeRoles.size) analysesInProgress.delete(token);
}

function resetReferenceAnalysisLimitsForTests() {
  analysesInProgress.clear();
  globalAnalysesInProgress = 0;
}

function noStore(req, res, next) {
  res.setHeader('Cache-Control', 'no-store');
  res.setHeader('Pragma', 'no-cache');
  next();
}

function jsonError(res, status, error) {
  res.status(status).json({ error });
}

/** Minimal cookie parser — avoids adding a cookie-parser dependency. */
function parseCookies(req) {
  const header = req.headers.cookie;
  const out = {};
  if (!header) return out;
  for (const part of header.split(';')) {
    const idx = part.indexOf('=');
    if (idx === -1) continue;
    const key = part.slice(0, idx).trim();
    const value = part.slice(idx + 1).trim();
    if (key) out[key] = decodeURIComponent(value);
  }
  return out;
}

function isRequestSecure(req) {
  if (req.secure) return true;
  const proto = String(req.headers['x-forwarded-proto'] || '').split(',')[0].trim();
  return proto === 'https';
}

function setSessionCookie(req, res, token, expiresAt) {
  const maxAgeSeconds = Math.max(0, Math.floor((expiresAt - Date.now()) / 1000));
  const attrs = [
    `${session.COOKIE_NAME}=${encodeURIComponent(token)}`,
    'Path=/rida-studio',
    'HttpOnly',
    'SameSite=Strict',
    `Max-Age=${maxAgeSeconds}`,
  ];
  if (isRequestSecure(req)) attrs.push('Secure');
  res.setHeader('Set-Cookie', attrs.join('; '));
}

function clearSessionCookie(req, res) {
  const attrs = [
    `${session.COOKIE_NAME}=`,
    'Path=/rida-studio',
    'HttpOnly',
    'SameSite=Strict',
    'Max-Age=0',
  ];
  if (isRequestSecure(req)) attrs.push('Secure');
  res.setHeader('Set-Cookie', attrs.join('; '));
}

function getSessionToken(req) {
  return parseCookies(req)[session.COOKIE_NAME] || null;
}

function requireAuth(req, res, next) {
  const token = getSessionToken(req);
  if (!session.isValid(token)) {
    return jsonError(res, 401, 'Not authenticated. Please log in with the studio PIN.');
  }
  req.ridaSessionToken = token;
  next();
}

function requireJsonBody(req, res, next) {
  if (!req.is('application/json')) {
    return jsonError(res, 415, 'Request body must be application/json.');
  }
  next();
}

function parseReferencePhoto(value, label) {
  if (value === undefined || value === null) return null;
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new Error(`${label} must be an image object.`);
  }
  const extraKeys = Object.keys(value).filter((key) => !['mimeType', 'base64'].includes(key));
  if (extraKeys.length) {
    throw new Error(`${label} contains unexpected fields.`);
  }
  if (!FABRIC_MIME_TYPES.has(value.mimeType)) {
    throw new Error(`${label} must be JPEG, PNG, or WebP.`);
  }
  if (typeof value.base64 !== 'string' || !value.base64 || value.base64.length % 4 !== 0 ||
      !/^[A-Za-z0-9+/]+={0,2}$/.test(value.base64)) {
    throw new Error(`${label} data is invalid.`);
  }
  const buffer = Buffer.from(value.base64, 'base64');
  if (buffer.length < 32 || buffer.length > MAX_FABRIC_BYTES) {
    throw new Error(`${label} must be between 32 bytes and ${MAX_FABRIC_BYTES / (1024 * 1024)} MB.`);
  }

  const isJpeg = buffer.length >= 3 && buffer[0] === 0xff && buffer[1] === 0xd8 && buffer[2] === 0xff;
  const isPng = buffer.length >= 8 &&
    buffer.subarray(0, 8).equals(Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]));
  const isWebp = buffer.length >= 12 &&
    buffer.subarray(0, 4).toString('ascii') === 'RIFF' &&
    buffer.subarray(8, 12).toString('ascii') === 'WEBP';
  if ((value.mimeType === 'image/jpeg' && !isJpeg) ||
      (value.mimeType === 'image/png' && !isPng) ||
      (value.mimeType === 'image/webp' && !isWebp)) {
    throw new Error(`${label} contents do not match its image type.`);
  }

  const extension = value.mimeType === 'image/png' ? 'png' : value.mimeType === 'image/webp' ? 'webp' : 'jpg';
  return {
    buffer,
    filename: `uploaded-${label.toLowerCase().replace(/[^a-z]+/g, '-')}.${extension}`,
    mimeType: value.mimeType,
  };
}

function selectIdentityReferences(identityReferences, hasUploadedReferences) {
  if (!hasUploadedReferences ||
      identityReferences.length <= MAX_IDENTITY_REFERENCES_WITH_UPLOADS) {
    return identityReferences;
  }

  const supporting = identityReferences.slice(1);
  const supportingCount = MAX_IDENTITY_REFERENCES_WITH_UPLOADS - 1;
  const selectedSupporting = Array.from({ length: supportingCount }, (_, index) => {
    const sourceIndex = supportingCount === 1
      ? 0
      : Math.round(index * (supporting.length - 1) / (supportingCount - 1));
    return supporting[sourceIndex];
  });
  return [identityReferences[0], ...selectedSupporting];
}

function createRouter() {
  const router = express.Router();
  router.use(express.json({ limit: '16mb' }));
  router.use(noStore);

  router.post('/login', requireJsonBody, (req, res) => {
    const pin = req.body && req.body.pin;
    if (!session.isPinConfigured()) {
      return jsonError(res, 503, 'The studio PIN is not configured on the server yet.');
    }
    let result;
    try {
      result = session.login(pin);
    } catch (err) {
      if (err.code === 'PIN_RATE_LIMITED') {
        const retryAfterSeconds = Math.max(1, Math.ceil(err.retryAfterMs / 1000));
        res.setHeader('Retry-After', String(retryAfterSeconds));
        return jsonError(res, 429, 'Too many incorrect PIN attempts. Please try again later.');
      }
      return jsonError(res, 503, 'The studio PIN is not configured on the server yet.');
    }
    if (!result) {
      return jsonError(res, 401, 'Incorrect PIN.');
    }
    setSessionCookie(req, res, result.token, result.expiresAt);
    res.json({ ok: true });
  });

  router.post('/logout', (req, res) => {
    const token = getSessionToken(req);
    if (token) session.logout(token);
    clearSessionCookie(req, res);
    res.json({ ok: true });
  });

  router.get('/session', (req, res) => {
    const token = getSessionToken(req);
    const authenticated = session.isValid(token);
    res.json({
      authenticated,
    });
  });

  router.get('/options', requireAuth, (req, res) => {
    res.json({
      options: options.listOptions(),
    });
  });

  router.post('/analyze-reference', requireAuth, requireJsonBody, async (req, res) => {
    const body = req.body || {};
    const extraKeys = Object.keys(body).filter((key) => !['role', 'photo'].includes(key));
    if (extraKeys.length) {
      return jsonError(res, 400, `Unexpected field(s) in request: ${extraKeys.join(', ')}`);
    }
    let role;
    let reference;
    try {
      role = referenceSpec.normalizeReferenceRole(body.role);
      reference = parseReferencePhoto(body.photo, 'Reference photo');
    } catch (err) {
      return jsonError(res, 400, err.message);
    }
    if (!reference) return jsonError(res, 400, 'A reference photo is required.');

    const apiKey = process.env.OPENAI_API_KEY;
    if (!apiKey) {
      return jsonError(res, 503, 'Reference analysis is not configured on the server yet.');
    }
    if (!beginReferenceAnalysis(req.ridaSessionToken, role)) {
      return jsonError(
        res,
        409,
        'Too many reference analyses are already running for this session. Please wait a moment.',
      );
    }
    const controller = new AbortController();
    res.on('close', () => {
      if (!res.writableEnded) controller.abort();
    });
    try {
      const result = await referenceAnalyzer.analyzeReference({
        apiKey,
        model: process.env.OPENAI_RIDA_ANALYSIS_MODEL,
        reference,
        role,
        signal: controller.signal,
      });
      if (controller.signal.aborted || res.writableEnded) return;
      const token = analysisToken.createAnalysisToken({
        sessionToken: req.ridaSessionToken,
        role,
        photoBuffer: reference.buffer,
        analysis: result.analysis,
      });
      res.json({ analysis: result.analysis, analysisToken: token });
    } catch (err) {
      if (err.code === 'OPENAI_ABORTED' || controller.signal.aborted) return;
      if (err.code === 'OPENAI_TIMEOUT') return jsonError(res, 504, err.message);
      return jsonError(res, 502, `Reference analysis failed: ${err.message}`);
    } finally {
      endReferenceAnalysis(req.ridaSessionToken, role);
    }
  });

  router.post('/generate', requireAuth, requireJsonBody, async (req, res) => {
    const body = req.body || {};

    // Exact allowlisted fields only — no unrestricted prompt text, no extras.
    const extraKeys = Object.keys(body).filter((k) => !SELECTION_FIELDS.includes(k));
    if (extraKeys.length) {
      return jsonError(res, 400, `Unexpected field(s) in request: ${extraKeys.join(', ')}`);
    }
    let designMode;
    try {
      designMode = promptBuilder.normalizeDesignMode(body.designMode);
    } catch (err) {
      return jsonError(res, 400, err.message);
    }
    const selectionErrors = promptBuilder.validateSelectionsForMode(body, designMode);
    if (selectionErrors.length) {
      return jsonError(res, 400, `Invalid selections: ${selectionErrors.join('; ')}`);
    }
    let completeRidaDescription;
    try {
      if (designMode === 'complete') {
        completeRidaDescription = promptBuilder.normalizeDescription(
          body.completeRidaDescription,
          'Complete-rida description',
        );
      } else {
        promptBuilder.normalizeDescription(body.baseDescription, 'Base-cloth description');
        promptBuilder.normalizeDescription(body.designDescription, 'Design description');
        promptBuilder.normalizeDescription(body.embroideryDescription, 'Embroidery description');
      }
    } catch (err) {
      return jsonError(res, 400, err.message);
    }

    let completeRidaReference;
    let baseClothReference;
    let designReference;
    let completeRidaAnalysis;
    let baseClothAnalysis;
    let designAnalysis;
    try {
      if (designMode === 'complete') {
        completeRidaReference = parseReferencePhoto(body.completeRidaPhoto, 'Complete rida photo');
        completeRidaAnalysis = completeRidaReference
          ? analysisToken.verifyAnalysisToken({
            token: body.completeRidaAnalysisToken,
            sessionToken: req.ridaSessionToken,
            role: 'complete',
            photoBuffer: completeRidaReference.buffer,
          })
          : null;
      } else {
        baseClothReference = parseReferencePhoto(body.baseClothPhoto, 'Base cloth photo');
        designReference = parseReferencePhoto(body.designPhoto, 'Design example photo');
        baseClothAnalysis = baseClothReference
          ? analysisToken.verifyAnalysisToken({
            token: body.baseClothAnalysisToken,
            sessionToken: req.ridaSessionToken,
            role: 'base_cloth',
            photoBuffer: baseClothReference.buffer,
          })
          : null;
        designAnalysis = designReference
          ? analysisToken.verifyAnalysisToken({
            token: body.designAnalysisToken,
            sessionToken: req.ridaSessionToken,
            role: 'design',
            photoBuffer: designReference.buffer,
          })
          : null;
      }
    } catch (err) {
      return jsonError(res, 400, err.message);
    }
    try {
      if (designMode === 'complete') {
        promptBuilder.normalizeDescription(body.completeRidaCorrection, 'Complete-rida correction');
      } else {
        promptBuilder.normalizeDescription(body.baseClothCorrection, 'Base-cloth correction');
        promptBuilder.normalizeDescription(body.designCorrection, 'Design correction');
      }
    } catch (err) {
      return jsonError(res, 400, err.message);
    }
    if (designMode === 'complete' && !completeRidaReference && !completeRidaDescription) {
      return jsonError(
        res,
        400,
        'Complete mode requires a complete-rida photo or nonempty complete-rida description.',
      );
    }

    const gate = rateLimit.beginGeneration(RATE_LIMIT_KEY);
    if (!gate.ok) {
      return jsonError(res, 409, 'A generation is already in progress for this session. Please wait for it to finish.');
    }

    let success = false;
    try {
      let manifestNames;
      try {
        manifestNames = tilesPhotos.readTilesManifest();
      } catch (err) {
        return jsonError(res, 500, 'Could not read the photo manifest.');
      }

      let identityResult;
      try {
        identityResult = identity.resolveIdentityPhotos(manifestNames);
      } catch (err) {
        // Never leak filenames or path details to the browser.
        return jsonError(res, 500, 'Identity reference photos are not configured correctly on the server.');
      }

      let identityReferences;
      try {
        identityReferences = identityResult.resolved.map((p) => ({
          buffer: fs.readFileSync(p.path),
          filename: p.name,
          mimeType: tilesPhotos.extForPhoto(p.name),
        }));
      } catch (err) {
        return jsonError(res, 500, 'Could not read identity reference photo(s).');
      }

      // Reference order matters for image-edit fidelity. Uploaded visual
      // sources come first so the model cannot underweight them behind ten
      // identity photos. A smaller, distributed identity set still preserves
      // Fatema without letting reference clothing dominate the requested
      // fabric or design.
      const hasUploadedReferences = Boolean(
        completeRidaReference || baseClothReference || designReference,
      );
      const selectedIdentityReferences = selectIdentityReferences(
        identityReferences,
        hasUploadedReferences,
      );
      const referenceBuffers = [];
      let nextReferenceIndex = 1;
      const completeRidaReferenceIndex = completeRidaReference ? nextReferenceIndex++ : null;
      if (completeRidaReference) referenceBuffers.push(completeRidaReference);
      const baseClothReferenceIndex = baseClothReference ? nextReferenceIndex++ : null;
      if (baseClothReference) referenceBuffers.push(baseClothReference);
      const designReferenceIndex = designReference ? nextReferenceIndex++ : null;
      if (designReference) referenceBuffers.push(designReference);
      const primaryIdentityReferenceIndex = nextReferenceIndex++;
      referenceBuffers.push(selectedIdentityReferences[0]);
      const supportingIdentityStartIndex = nextReferenceIndex;
      referenceBuffers.push(...selectedIdentityReferences.slice(1));

      let prompt;
      try {
        prompt = promptBuilder.buildPrompt({
          ...body,
          completeRidaAnalysis,
          baseClothAnalysis,
          designAnalysis,
        }, {
          identityReferenceCount: selectedIdentityReferences.length,
          hasCompleteRidaReference: Boolean(completeRidaReference),
          hasBaseClothReference: Boolean(baseClothReference),
          hasDesignReference: Boolean(designReference),
          completeRidaReferenceIndex,
          baseClothReferenceIndex,
          designReferenceIndex,
          primaryIdentityReferenceIndex,
          supportingIdentityStartIndex,
        });
      } catch (err) {
        return jsonError(res, 400, err.message);
      }

      const apiKey = process.env.OPENAI_API_KEY;
      if (!apiKey) {
        return jsonError(res, 503, 'Image generation is not configured on the server yet. Please try again later.');
      }

      let result;
      try {
        result = await openaiImagesClient.generateImageEdits({
          apiKey,
          model: process.env.OPENAI_IMAGE_MODEL || openaiImagesClient.DEFAULT_MODEL,
          referenceBuffers,
          prompt,
          n: GENERATE_COUNT,
          size: IMAGE_SIZE,
          quality: IMAGE_QUALITY,
          outputFormat: 'png',
        });
      } catch (err) {
        if (err.code === 'OPENAI_TIMEOUT') {
          return jsonError(res, 504, err.message);
        }
        return jsonError(res, 502, `Image generation failed: ${err.message}`);
      }

      if (!Array.isArray(result.images) || result.images.length !== GENERATE_COUNT) {
        return jsonError(res, 502, 'Image generation returned an unexpected number of results.');
      }

      success = true;
      rateLimit.endGeneration(RATE_LIMIT_KEY);
      res.json({
        images: result.images.map((img) => img.b64Json),
        model: result.model,
      });
    } finally {
      // Release the concurrency lock when an early return or API failure
      // skipped the success path.
      if (!success) rateLimit.endGeneration(RATE_LIMIT_KEY);
    }
  });

  // Explicit JSON 404 for anything unmatched under this router.
  router.use((req, res) => {
    jsonError(res, 404, `Not found: ${req.method} ${req.path}`);
  });

  // eslint-disable-next-line no-unused-vars
  router.use((err, req, res, next) => {
    if (err && err.type === 'entity.too.large') {
      return jsonError(res, 413, 'The uploaded reference photo is too large.');
    }
    jsonError(res, 500, `Unexpected server error: ${err.message}`);
  });

  return router;
}

module.exports = {
  createRouter,
  GENERATE_COUNT,
  IMAGE_SIZE,
  IMAGE_QUALITY,
  SELECTION_FIELDS,
  RATE_LIMIT_KEY,
  MAX_FABRIC_BYTES,
  MAX_IDENTITY_REFERENCES_WITH_UPLOADS,
  FABRIC_MIME_TYPES,
  MAX_CONCURRENT_ANALYSES_PER_SESSION,
  MAX_CONCURRENT_ANALYSES_GLOBAL,
  beginReferenceAnalysis,
  endReferenceAnalysis,
  _resetReferenceAnalysisLimitsForTests: resetReferenceAnalysisLimitsForTests,
  parseReferencePhoto,
  selectIdentityReferences,
  parseCookies,
  getSessionToken,
};
