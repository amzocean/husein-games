...
const FLEXIBLE_IDENTITY_CLAUSE =
  ...
  "first reference as the primary facial reference only and the remaining references as supporting views " +
  "of the same identity. Preserve her distinctive face shape, eyes, eyebrows, nose, lips, smile, " +
  "natural skin complexion, approximate age, natural adult body build, height impression, and normal " +
  "facial and body proportions. Do not make her thinner, heavier, taller, shorter, younger, or older. " +
  "In realistic work, retain " +
...
const FRESH_RIDA_CLAUSE =
  "Treat all clothing visible in every attached reference photo as incidental identity context, not as " +
  "a wardrobe or rida-design reference. Do not copy, reconstruct, or reuse any reference rida's color " +
  "palette, fabric, print, motifs, borders, lace, embroidery, panel layout, or embellishments unless " +
  "Fatema's creative direction explicitly asks to reproduce that particular outfit. Preserve the " +
  "authentic rida form and coverage, but create a fresh rida rendition for every generation. Follow any " +
  "colors or design details Fatema requests; when she does not specify them, independently choose a new, " +
  "harmonious color palette, fabric treatment, motifs, borders, and embellishments. The two generated " +
  "candidates must use visibly different rida colors and design details from each other and from all " +
  "reference outfits while keeping Fatema's face, identity, body, and authentic rida attire consistent.";
...
function buildPrompt(request, { identityReferenceCount = 0 } = {}) {
  ...
  const referencePhrase = identityReferenceCount > 0
    ? `Use the first ${identityReferenceCount} attached reference photos as the exclusive ground truth for Fatema's identity and natural body proportions only, never for wardrobe design.`
    : '';
