...
function createRouter({
  ...
  router.post('/generate', requireAuth, requireJsonBody, async (req, res) => {
    ...
    if (!gate.ok) {
      ...
      return jsonError(res, 429, `Daily generation limit reached (${rateLimit.DAILY_LIMIT} shared studio generations per UTC day). Please try again tomorrow.`);
    }
    ...
    try {
      ...
      success = true;
      rateLimit.endGeneration(RATE_LIMIT_KEY, { success: true });
      res.json({
