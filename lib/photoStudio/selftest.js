...
async function main() {
  ...
  test('uses references for identity and body but invents fresh rida designs', () => {
    const prompt = promptBuilder.buildPrompt(
      'Create a joyful portrait in a beautiful garden.',
      { identityReferenceCount: 10 },
    );
    assert.ok(prompt.includes('identity and natural body proportions only, never for wardrobe design'));
    assert.ok(prompt.includes('primary facial reference only'));
    assert.ok(prompt.includes('Do not make her thinner, heavier, taller, shorter, younger, or older'));
    assert.ok(prompt.includes('all clothing visible in every attached reference photo as incidental identity context'));
    assert.ok(prompt.includes('create a fresh rida rendition for every generation'));
    assert.ok(prompt.includes('when she does not specify them, independently choose a new'));
    assert.ok(prompt.includes('candidate must use a fresh rida color palette and design'));
  });
  ...
  test('shares the Rida Studio concurrency key', () => {
    assert.strictEqual(photoRouter.RATE_LIMIT_KEY, ridaRouter.RATE_LIMIT_KEY);
  });

  test('uses one medium-quality portrait output and an exact request allowlist', () => {
    assert.strictEqual(photoRouter.GENERATE_COUNT, 1);
    assert.strictEqual(photoRouter.IMAGE_SIZE, '1024x1536');
    assert.strictEqual(photoRouter.IMAGE_QUALITY, 'medium');
  ...
  try {
    ...
    await testAsync('generates one medium-quality image from ten identity references', async () => {
      const response = await fetch(`${base}/generate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Cookie: cookie },
        body: JSON.stringify({
      ...
      assert.strictEqual(fakeRequest.n, 1);
      assert.strictEqual(fakeRequest.size, '1024x1536');
      assert.strictEqual(fakeRequest.quality, 'medium');
